...
router.get('/', (req, res) => {
    MongoClient.connect('mongodb://lurker:gaisma@46.101.142.29:27020/users', async (err, client) => {
        ...
        const high_instrucor_sa = await db.collection('users').find({ position: "High Instructor SA" }).toArray();
        const instructor_sa = await db.collection('users').find({ position: "Instructor SA" }).toArray();

        const sa = curator_sa.concat(head_sa).concat(d_head_sa).concat(high_instrucor_sa).concat(instructor_sa);
